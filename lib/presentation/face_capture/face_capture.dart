...
Float32List byteArrayToFloatArray(Uint8List byteArray) {
  ...
  Float32List floatArray = Float32List(
    byteArray.length ~/ Float32List.bytesPerElement,
  );
  ...
  for (int i = 0; i < byteArray.length; i += Float32List.bytesPerElement) {
    ...
    floatArray[i ~/ Float32List.bytesPerElement] = byteArray.buffer
        .asFloat32List()[i ~/ Float32List.bytesPerElement];
  }
...
class FaceCaptureView extends StatefulWidget {
  ...
  FaceCaptureView({super.key});

  @override
  State<FaceCaptureView> createState() => FaceCaptureViewState();
}
...
class FaceCaptureViewState extends State<FaceCaptureView> {
  ...
  double identifyThresholds = 0;
  bool recognizedd = false;
  ...
  Future<bool> onFaceDetected(faces) async {
    ...
    if (_viewMode == ViewMode.REPEAT_NO_FACE_PREPARE) {
      ...
      //   });
      // }
      else if (faceCaptureState.index == FaceCaptureState.MOUTH_OPENED.index) {
        setState(() {
        ...
        // createClipedImage();
        ui.Image capturedImage = await decodeImageFromList(
          _currentFace['faceJpg'],
        );

        Rect roiRectSrc = Rect.fromLTWH(
          0,
          0,
  ...
  FaceCaptureState checkFace(faces) {
    ...
    try {
      if (Platform.isAndroid) {
        ...
        for (int i = 0; i < 68; i++) {
          faceLeft = faceLeft < landmarks_68[i * 2]
              ? faceLeft
              : landmarks_68[i * 2];
          faceRight = faceRight > landmarks_68[i * 2]
              ? faceRight
              : landmarks_68[i * 2];
        ...
        List landmarks_68 = byteArrayToFloatArray(
          Uint8List.fromList(face['landmarks_68']),
        );
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: <Widget>[
            ...
            Visibility(
              visible: _viewMode.index == ViewMode.FACE_CAPTURE_FINISHED.index,
              child: SizedBox(
                width: double.infinity,
                height: double.infinity,
                      ...
                      Padding(
                        ...
                        child: SriButton(
                          ...
                          onPressed: () {
                            Get.back(
                              result: {
                                "face_URL": _capturedFace["faceJpg"],
                                "face_template": _capturedFace["templates"],
                              },
                            );
                          },
                        ),
...
class CapturePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) async {
    ...
    Paint outSideActiveRoundPaint = Paint()
      ..style = PaintingStyle
          .stroke // Equivalent to Paint.Style.STROKE in Java
      ..strokeWidth =
          4 // Set stroke width
      ..color =
          const Color(
            0xFFE6E1E5,
          ) // Set the color (use the actual color value or from your theme)
    ...
    if (viewMode == ViewMode.FACE_CIRCLE ||
      ...
      double startWidth =
          0.8 * roiViewRect.width * 0.5 / math.cos(45 * math.pi / 180);
      ...
      if (viewMode != ViewMode.FACE_CAPTURE_PREPARE) {
        ...
        final borderPaint = Paint()
          ..style = PaintingStyle.fill
          ..color =
              const Color(0xFF492532) // Replace with the correct color
          ..isAntiAlias = true;
    ...
    if (viewMode == ViewMode.NO_FACE_PREPARE ||
      ...
      if (viewMode == ViewMode.NO_FACE_PREPARE ||
          (viewMode == ViewMode.TO_FACE_CIRCLE && animateValue > 1.0)) {
        int alpha = (math.min(255, ((1.4 - animateValue) / 0.4 * 255))).toInt();
      ...
      if (viewMode == ViewMode.NO_FACE_PREPARE ||
        ...
        path1.moveTo(
          scaleRect.left,
          scaleRect.top + lineHeight1 + lineHeightOffset1,
        );
        path1.lineTo(scaleRect.left, scaleRect.top + quadR1);
        path1.arcTo(
          Rect.fromLTWH(scaleRect.left, scaleRect.top, quadR1 * 2, quadR1 * 2),
          math.pi,
          math.pi / 2,
          false,
        );
        path1.lineTo(
          scaleRect.left + lineWidth1 + lineWidthOffset1,
          scaleRect.top,
        );
        ...
        path2.lineTo(
          scaleRect.right - lineWidth1 - lineWidthOffset1,
          scaleRect.top,
        );
    ...
    if (viewMode == ViewMode.FACE_CIRCLE) {
      ...
      for (int i = 0; i < 360; i += 5) {
        ...
        double x1 =
            ...
            b1 /
            math.sqrt(
              math.pow(b1, 2) + math.pow(a1, 2) * math.tan(th) * math.tan(th),
            );
        double x2 =
            a2 *
            b2 /
            math.sqrt(
              math.pow(b2, 2) + math.pow(a2, 2) * math.tan(th) * math.tan(th),
            );
      ...
      if (currentFace != null) {
        final Paint paint1 = Paint()
          ...
          ..color =
              const Color(
                0x80EADDFF,
              ) // Replace with actual color value or use a theme color
          ..isAntiAlias = true;
        ...
        final Path path1 = Path()
          ...
          ..quadraticBezierTo(
            roiViewRect.center.dx -
                roiViewRect.width * (math.sin(yaw * math.pi / 180)),
          ...
          ..quadraticBezierTo(
            roiViewRect.center.dx -
                roiViewRect.width * (math.sin(yaw * math.pi / 180)) / 3,
            roiViewRect.center.dy,
            roiViewRect.center.dx,
            roiViewRect.top,
          );
        ...
        final Path path2 = Path()
          ...
          ..quadraticBezierTo(
            roiViewRect.center.dx,
            roiViewRect.center.dy +
                roiViewRect.width * (math.sin(pitch * math.pi / 180)),
            roiViewRect.right,
            roiViewRect.center.dy,
          )
          ..quadraticBezierTo(
            roiViewRect.center.dx,
            roiViewRect.center.dy +
                roiViewRect.width * (math.sin(pitch * math.pi / 180)) / 3,
            roiViewRect.left,
            roiViewRect.center.dy,
          );
      ...
      if (currentFace != null) {
        ...
        final Paint paint1 = Paint()
          ..style = PaintingStyle.stroke
          ..color = Colors
              .tealAccent // Replace with your actual color
          ..strokeWidth = 7
          ..isAntiAlias = true;
