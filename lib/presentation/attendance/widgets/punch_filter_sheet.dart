...
class PunchFilterSheet extends StatefulWidget {
  final AttendanceController controller;
  const PunchFilterSheet({super.key, required this.controller});

  @override
  State<PunchFilterSheet> createState() => _PunchFilterSheetState();
}

class _PunchFilterSheetState extends State<PunchFilterSheet> {
  late DateTime? from;
  late DateTime? to;
  String? empId;
  ...
  void initState() {
    ...
    from = widget.controller.punchFromDate.value;
    to = widget.controller.punchToDate.value;
    empId = !auth.isAdmin
        ? auth.employeeId
        : widget.controller.punchFilterEmployeeId.value;
    empCtrl = Get.find<EmployeeController>();
    selectedPreset = widget.controller.punchActivePreset.value;
  }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Row(
              children: [
                Icon(Icons.tune_outlined, color: AppColors.warning, size: 18),
                SizedBox(width: 8),
                Text(
                  'Filter Punch Adjustments',
                  style: TextStyle(
                    fontSize: 16,
            ...
            if (auth.isAdmin)
              Obx(() {
                ...
                final ids = allEmps.map((e) => e.id).toList();
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    onPressed: () {
                      widget.controller.clearPunchFilters();
                ...
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      widget.controller.applyPunchFilters(
                        from: from,
                        to: to,
                        employeeId: empId ?? '',
